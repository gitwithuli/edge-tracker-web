                  ...
                  <div key={log.id} className="relative pl-6 border-l border-zinc-800">
                    ...
                    <div className="bg-zinc-900/40 p-4 rounded-lg border border-zinc-800/50 hover:bg-zinc-900/60 transition-colors">
                      <div className="flex justify-between items-start mb-3">
                        <Badge variant="outline" className={cn("px-2 py-0.5 font-bold", log.result === "WIN" ? "text-green-400 border-green-900/50" : "text-red-400 border-red-900/50")}>
                          {log.result}
                        </Badge>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="h-6 w-6 p-0 hover:bg-zinc-800">
                              <MoreHorizontal className="w-4 h-4 text-zinc-500" />
                            </Button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end" className="bg-zinc-900 border-zinc-800 text-zinc-200">
                             <LogDialog initialData={log} onSave={(newData) => onUpdateLog(String(log.id), newData)} trigger={<div className="relative flex cursor-pointer items-center rounded-sm px-2 py-1.5 text-sm hover:bg-zinc-800"><Pencil className="mr-2 h-4 w-4" /> Edit</div>} />
                            <DropdownMenuItem className="text-red-400 cursor-pointer" onClick={() => { if (confirm("Delete?")) onDeleteLog(log.id); }}><Trash2 className="mr-2 h-4 w-4" /> Delete</DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>

                      {imageUrl && (
                        <div className="mt-2 mb-3 group relative overflow-hidden rounded-md border border-zinc-800 bg-zinc-950">
                          <Dialog>
                            <DialogTrigger asChild>
                              <div className="cursor-zoom-in relative aspect-video">
                                <img src={imageUrl} alt="Preview" className="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-all" />
                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40"><ZoomIn className="w-5 h-5 text-white" /></div>
                              </div>
                            </DialogTrigger>
                            <DialogContent className="max-w-[95vw] max-h-[90vh] p-0 bg-transparent border-none flex items-center justify-center">
                              <img src={imageUrl} alt="Zoomed" className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl border border-zinc-800" />
                            </DialogContent>
                          </Dialog>
                        </div>
                      )}
                      ...
                      {log.note && <p className="text-sm text-zinc-400 mt-2 italic border-l border-zinc-800 pl-2">"{log.note}"</p>}
                    </div>
                  </div>
                );
              })
