...
export const LogDialog = memo(function LogDialog({ edgeName, initialData, trigger, onSave }: LogDialogProps) {
  ...
  const [logType, setLogType] = useState<LogType>(initialData?.logType as LogType || DEFAULT_LOG_VALUES.logType);
  ...
  useEffect(() => {
    if (open) {
      if (initialData) {
        ...
        setLogType(initialData.logType as LogType || 'FRONTTEST');
        ...
        setLogType(DEFAULT_LOG_VALUES.logType);
